...
return new class extends Migration
{
    ...
    {
        Schema::create('solar_plant_batteries', function (Blueprint $table) {
            $table->id();
            $table->string('serial')->unique();
            $table->foreignId('manufacturer_user_id')->constrained('users');
            $table->integer('production_year');
            $table->integer('expiration_year');
            $table->string('status')->default(BatteryStatus::PENDING_APPROVAL->value);
    ...
    {
        Schema::dropIfExists('solar_plant_batteries');
    }
};
