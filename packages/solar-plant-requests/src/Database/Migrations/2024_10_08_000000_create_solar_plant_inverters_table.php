...
return new class extends Migration
{
    ...
    {
        Schema::create('solar_plant_inverters', function (Blueprint $table) {
            $table->id();
            $table->string('serial')->unique();
            $table->foreignId('manufacturer_user_id')->constrained('users');
            $table->integer('production_year');
            $table->integer('expiration_year');
            $table->string('status')->default(InverterStatus::PENDING_APPROVAL->value);
    ...
    {
        Schema::dropIfExists('solar_plant_inverters');
    }
};
